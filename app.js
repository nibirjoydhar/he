...
const App = () => {
    ...
    const handleSubmit = (e) => {
        ...
        .then(res => res.json())
        .then(data => {
            document.getElementById('contactResponse').innerHTML = '<div class="text-green-400">Message sent successfully!</div>';
            form.reset();
        })
        .catch(err => {
            document.getElementById('contactResponse').innerHTML = '<div class="text-red-400">Something went wrong!</div>';
        });
    };
        ...
        <div>
            ...
            <header className="fixed top-0 w-full bg-black bg-opacity-80 z-50">
                <nav className="container mx-auto px-4 py-4 flex items-center justify-between">
                    <a href="/" className="text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-green-400 to-pink-500">Nibir Joydhar</a>
                ...
                {isMenuOpen && (
                    <div className="md:hidden bg-black bg-opacity-90">
                        {['hero', 'about', 'skills', 'projects', 'achievements', 'online-judges', 'education', 'book-me', 'contact'].map(section => (
                            <a key={section} href={`#${section}`} className="block px-4 py-2 nav-link" onClick={() => setIsMenuOpen(false)}>
            ...
            <section id="hero" className="min-h-screen flex items-center justify-center relative overflow-hidden">
                ...
                <div className="container mx-auto px-4 text-center z-10">
                    <img src="images/nibir.png" className="w-40 h-40 rounded-full mx-auto mb-6 border-4 border-green-400 shadow-lg animate-pulse" alt="Nibir Joydhar" />
                    <h1 className="text-6xl md:text-8xl font-extrabold mb-4 text-transparent bg-clip-text bg-gradient-to-r from-green-400 to-pink-500">Nibir Joydhar</h1>
                    <p className="text-2xl md:text-3xl mb-6">Competitive Programmer | Full Stack Developer | Tech Enthusiast</p>
                    <p className="text-lg mb-8">Specialist on Codeforces | MERN & Laravel Developer | ICPC & IEEEXtreme Contestant</p>
                    <div className="flex justify-center gap-6">
                        <a href="https://flowcv.com/resume/8hhssgqlk5" target="_blank" className="btn btn-primary">View CV</a>
                        <a href="#book-me" className="btn btn-secondary">Hire Me</a>
                    </div>
                    <div className="flex justify-center gap-8 mt-8">
                        {['linkedin', 'github', 'facebook', 'instagram', 'x', 'envelope'].map((platform, index) => (
                            <a key={platform} href={platform === 'envelope' ? 'mailto:nibirjoydhar@gmail.com' : `https://${platform}.com/nibirjoydhar`} target="_blank">
                                <i className={`fa${platform === 'envelope' ? 's' : 'b'} fa-${platform} fa-2x hover:text-green-400 transition-transform duration-300 hover:scale-125`}></i>
                            </a>
                        ))}
                    </div>
                </div>
            ...
            <section id="book-me" className="py-16 bg-black bg-opacity-90">
                <div className="container mx-auto px-4">
                    ...
                    <p className="text-lg mb-8 text-center">Interested in collaborating or discussing a project? Schedule a 30-minute consultation to explore how I can help with your development needs.</p>
                    <div className="book-me-container">
                        <div className="calendly-inline-widget" data-url="https://calendly.com/nibirjoydhar/30min" style={{ minWidth: '320px', height: '600px' }}></div>
                        <a href="https://calendly.com/nibirjoydhar/30min" target="_blank" className="btn btn-primary mt-4">Schedule on Calendly</a>
                        <p className="text-sm mt-4 text-gray-400">If the widget doesn't load, click the button above to book directly.</p>
                    </div>
                </div>
            </section>

            {/* Contact Section */}
            <section id="contact" className="py-16 bg-gradient-to-r from-green-500 to-pink-500">
                <div className="container mx-auto px-4">
                    <h2 className="section-title">Get in Touch</h2>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div>
                            <p><i className="fas fa-envelope mr-2"></i><a href="mailto:nibirjoydhar@gmail.com" className="text-white">nibirjoydhar@gmail.com</a></p>
                            <p><i className="fas fa-phone mr-2"></i><a href="tel:+8801521546883" className="text-white">+880 1521 546 883</a></p>
                            <p><i className="fas fa-map-marker-alt mr-2"></i>Jagannath University, Dhaka, Bangladesh</p>
                        </div>
                        <form id="contactForm" onSubmit={handleSubmit} action="https://formspree.io/f/xnnddzow" method="POST">
                            ...
                            <div id="contactResponse" className="mt-4"></div>
                        </form>
                    </div>
                </div>
            </section>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        <div>
                            ...
                            <ul className="space-y-2">
                                {['hero', 'about', 'skills', 'projects', 'achievements', 'online-judges', 'book-me', 'contact'].map(section => (
                                    <li key={section}><a href={`#${section}`} className="hover:text-green-400">{section.charAt(0).toUpperCase() + section.slice(1)}</a></li>
                                ))}
                            </ul>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <div className="flex gap-4">
                                {['linkedin', 'github', 'facebook', 'instagram', 'x'].map(platform => (
                                    <a key={platform} href={`https://${platform}.com/nibirjoydhar`} target="_blank">
